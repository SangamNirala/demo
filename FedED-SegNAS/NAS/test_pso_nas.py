...
def test_pso_optimizer():
    ...
    assert pso.gbest_position is None, "Initial gbest_position should be None"
    ...
    # Test that gbest was updated after search
    assert pso.gbest_fitness < float('inf'), "Gbest fitness should be updated after search"
    assert pso.gbest_position is not None, "Gbest position should be set after search"
...
def test_integration():
    ...
    # Import using sys.path manipulation since folder has space
    import sys
    import os
    phase3_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Phase 3')
