#!/usr/bin/env python3
"""
TASK 2 IMPLEMENTATION: Comprehensive Training with Increased Rounds - IMPROVED VERSION
=======================================================================================
...
- Robust early stopping with smoothing
- Comprehensive logging and monitoring

Key improvements over original:
- base_rounds is now properly enforced
...
Usage:
------
python train_all_models_comprehensive.py                # Train with 200 epochs (default)
python train_all_models_comprehensive.py --high-accuracy # 500 epochs with high accuracy
python train_all_models_comprehensive.py --quick        # Quick test with reduced rounds
python train_all_models_comprehensive.py --model model1 # Train specific model only

For 1000 epochs (paper configuration):
python train_all_models_comprehensive222.py             # Use the 222 version for 1000 epochs
...
class TrainingConfiguration:
    ...
    DEFAULT = {
        'min_rounds': 50,           # Minimum rounds (enforced!)
        'max_rounds': 200,          # Maximum rounds
        ...
        'warmup_rounds': 10,        # Warmup before decay
        'use_warmup': True,         # Enable warmup
        'use_nas': False,           # NAS disabled by default
        'nas_frequency': 15,        # NAS frequency if enabled
    ...
    HIGH_ACCURACY = {
        'min_rounds': 100,          # Minimum rounds before early stopping
        'max_rounds': 500,          # Higher maximum rounds
        'patience': 80,             # Early stopping patience (optimized based on analysis)
...
class ComprehensiveFederatedTrainer:
    ...
    def __init__(self, config=None, preset='default'):
        ...
        self.results_dir = Path(f'results/comprehensive_training_{timestamp}')
    ...
    def _print_configuration(self):
        ...
        print("="*80)
        print("COMPREHENSIVE FEDERATED TRAINING - IMPROVED VERSION")
        print("="*80)
        ...
        early_stop_status = "DISABLED" if self.config['patience'] >= self.config['max_rounds'] else "ENABLED"
        print(f"    - Early stopping: {early_stop_status} (patience: {self.config['patience']})")
    ...
    def train_single_dataset(self, model_name, order, num_snps, dataset_id, filepath):
        ...
        try:
            ...
            print(f"   Minimum rounds: {self.config['min_rounds']} (ENFORCED)")
            print(f"   Maximum rounds: {self.config['max_rounds']}")
            early_stop_status = "DISABLED" if self.config['patience'] >= self.config['max_rounds'] else f"ENABLED (patience: {self.config['patience']})"
            print(f"   Early stopping: {early_stop_status}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Comprehensive Federated Training - Improved Version',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
---------
  # Train all models with default configuration (200 epochs)
  ...
  # For 1000 epochs (paper configuration), use:
  python train_all_models_comprehensive222.py
  
  # Custom configuration
  python train_all_models_comprehensive.py --min-rounds 100 --max-rounds 300 --patience 30
  
  # Enable NAS
  python train_all_models_comprehensive.py --with-nas
        """
    )
    ...
    preset_group.add_argument(
        ...
        help='High accuracy configuration (500 epochs, slower but better results)'
    )
    preset_group.add_argument(
